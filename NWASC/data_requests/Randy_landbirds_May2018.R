...
landbirds = dbGetQuery(db,"select * from lu_species where species_type_id = 8")
...
write.csv(all_dat,"Z:/seabird_database/data_sent/Randy_landbirds_May2018/observations.csv")
...
write.csv(num_records,"Z:/seabird_database/data_sent/Randy_landbirds_May2018/num_records.csv")
...
ggplot(data =usa) + geom_polygon(aes(x = long, y = lat, group = group), fill = "#669933", color = "#333300") + 
  coord_fixed(1.3) + coord_fixed(xlim = c(-82, -50), ratio = 1.3) +
  geom_point(data = dat, aes(x = Long, y = Lat, col=spp_cd)) + theme_bw()+
  ggtitle("Lanbirds")
