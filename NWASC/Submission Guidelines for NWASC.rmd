...
2) If species codes were used, please include documentation describing the codes. For example, a separate column or spreadsheet defining COTE means common tern.  [ITIS](https://www.itis.gov/) codes are preferred. If you would like a list of our species codes we can supply one. We do accept observations for other marine fauna other than birds and boats.   
...
*    b. If the data are in multiple files, please make sure formatting between files is the same (e.g. tables and columns have the same names) so that they can easily be combined      
