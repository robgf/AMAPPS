...
# break by date
#-----------------#
x = obs_standard[obs_standard$date %in% obs_standard$date[1],]
y = track_standard[track_standard$date %in% obs_standard$date[1],]

# check for any existing effort from notes
z = x[x$spp %in% c("BEGCNT","ENDCNT"),]
...
#remove estimated start/stops where real ones occur 
z$zkey = paste(z$spp, z$transect, sep = "_")
sum.nn = sum.nn %>% mutate(zkey = paste(spp, transect, sep = "_")) %>% 
  filter(!zkey %in% z$zkey)
rm(z)
...
# export
write.csv(x, paste(dir.out, "/standard_obs_", as.character(y$date[1]), sep=""))
write.csv(y, paste(dir.out, "/standard_track_", as.character(y$date[1]), sep=""))
