...
# read in cts and observation tables
load("Q:/Kyle_Working_Folder/Segmentation/From_Arliss/database_extract_cts_obs.RData")

# read in transect shapefile
setwd("Q:/Kyle_Working_Folder/NWASC_data/transect_shp_files")
shp = readShapeLines("line")

preSegment = function(observations, tracks, transects) {

  # -------- prepare transect shapefile for segmentation --------------------------------------------------------------------------
  shp_tidy = tidy(tracks)
  shp_data = tracks@data[, c("transect_i", "dataset_id")]
  shp_data$id = seq.int(nrow(shp_data)) - 1
  
  # remove duplicate waypoints
  shp_tidy = shp_tidy %>% select(-order) %>% distinct %>% group_by(id) %>% mutate(order = seq.int(n())) %>% ungroup
  
  # merge continuous pieces in shapefile
  id_groups = shp_tidy %>% select(-c(group, order)) %>% mutate(id = as.integer(id), piece = as.integer(piece)) %>%
  ...
  shp_tidy_data = shp_tidy %>% mutate(id = as.integer(id),
                                      piece = as.integer(ifelse(group %in% id_groups$group, NA, piece))) %>%
    select(-group) %>% group_by(id) %>% mutate(piece = as.integer(factor(na.locf(piece)))) %>% ungroup %>%
    left_join(., shp_data, by = "id") %>% select(-id) %>% filter(dataset_id %in% transects$dataset_id) %>%
  ...
  id_transects = shp_tidy_data %>% select(-c(order, group)) %>%
  ...
  shp.pre <<- shp_tidy_data %>% mutate(new_transect_id = ifelse(group %in% id_transects$group, NA, transect_i),
                                               piece = ifelse(group %in% id_transects$group, NA, piece),
                                               order = ifelse(group %in% id_transects$group, NA, order)) %>%
  ...
  new_obs_transect_id = shp.pre %>% select(transect_id, new_transect_id) %>% distinct
  obs.dat.new = left_join(obs.time, cts.time, by = "transect_id") %>%
    ...
    left_join(., new_obs_transect_id, by = "transect_id") %>% rename(count = obs_count_intrans_nb) %>%
    ...
    filter(transect_id %in% transects$transect_id, spp_cd != "NONE", count >= 0, !is.na(st_astext))
  
  # create dataframe suitable to become spatial points object
